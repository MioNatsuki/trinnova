...
def _load_user(db: Session, user_id: int) -> Usuario:
    user = (
        db.query(Usuario)
        .options(joinedload(Usuario.rol), joinedload(Usuario.proyectos).joinedload("proyecto"))
...
def crear_usuario(
    ...
    for id_proy in payload.proyectos:
        proy = db.query(Proyecto).filter(Proyecto.id == id_proy).first()
        if proy:
            db.add(UsuarioProyecto(id_usuario=nuevo.id, id_proyecto=proy.id))
