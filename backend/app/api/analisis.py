...
# ============================================================
def _parse_fecha_pensiones(valor: Any) -> Optional[datetime]:
    """
    Convierte los diferentes formatos que puede tener una fecha de Pensiones
    a datetime.

    Retorna None si el valor está vacío o no se puede interpretar.
    """
    if valor is None:
        return None

    # pandas / numpy: NaN, NaT, etc.
    try:
        if pd.isna(valor):
            return None
    except (TypeError, ValueError):
    ...
    # pandas.Timestamp u objetos equivalentes
    ...
    if isinstance(valor, str):
        ...
        # Fallback para otros formatos válidos que pudieran venir del archivo
        try:
            from dateutil import parser
...
def _calcular_ultimo_abono_modificado(
    ultimo_abono: Any,
    ...
    - Si no existe último abono -> None.
    - Si el último abono tiene más de 5 años -> None.
    - Si tiene exactamente 5 años o menos -> conserva la fecha.

    La comparación es por años calendario, no por days / 365.25.
    """
    fecha_abono = _parse_fecha_pensiones(ultimo_abono)

    if fecha_abono is None:
        return None

    # Normalizar para comparar únicamente fechas.
    fecha_abono_date = fecha_abono.date()
    referencia_date = fecha_referencia.date()

    try:
        limite_cinco_anios = referencia_date.replace(
            year=referencia_date.year - 5
        )

    except ValueError:
        # Caso 29 de febrero:
        # 29/02/2024 - 5 años no existe en 2019.
        # Se utiliza 28/02/2019.
        limite_cinco_anios = referencia_date.replace(
            year=referencia_date.year - 5,
            day=28
        )

    if fecha_abono_date < limite_cinco_anios:
        return None

    return fecha_abono_date.isoformat()


def _calcular_sub_estatus_pensiones(
    programa: Any
) -> Optional[str]:
    """
    Determina el sub_estatus_id de Pensiones a partir del programa.

    CJ = Cobranza Judicial
    CE = Cobranza Extrajudicial
    """
    if programa is None:
        return None

    programa_normalizado = str(programa).strip().lower()

    if not programa_normalizado:
        return None

    # Quitar acentos para soportar, por ejemplo:
    # "Jurídico" y "Juridico".
    programa_normalizado = unicodedata.normalize(
        "NFKD",
        programa_normalizado
    )

    programa_normalizado = "".join(
        caracter
        for caracter in programa_normalizado
        if not unicodedata.combining(caracter)
    )

    indicadores_judiciales = (
        "judicial",
        "juridico",
        "cobranza juridica",
    )
    ...
    if any(
        indicador in programa_normalizado
        for indicador in indicadores_judiciales
    ):
        return "CJ"
        ...
        try:
            ...
            create_query = text("""
                CREATE TABLE IF NOT EXISTS tabla_dinamica (
                    ...
                    -- PENSIONES
                    -- ========================================================
                    ultimo_abono_modificado DATE NULL,
                    sub_estatus_id VARCHAR(2) NULL,
            ...
            try:
                ...
                if proyecto_slug == "estado":
                    ...
                    if not fecha_notificacion:
                        errores.append(
                            f"Fila {pk_value}: Falta fecha_notificacion - SKIP"
                        )
                        continue
                    ...
                    if isinstance(fecha_notificacion, str):
                        try:
                            ...
                            errores.append(
                                f"Fila {pk_value}: "
                                "Formato fecha_notificacion inválido - SKIP"
                            )
                            continue
                        ...
                        try:
                            fecha_notificacion = datetime.combine(
                                fecha_notificacion,
                                datetime.min.time()
                            )
                        except Exception:
                            errores.append(
                                f"Fila {pk_value}: "
                                "fecha_notificacion tipo inválido - SKIP"
                            )
                            continue
                    ...
                    total_actualizado = float(
                        data["total_actualizado"]
                    )

                    importe_letra = numero_a_letras(
                        total_actualizado
                    )
                    ...
                    data_dict = {
                        ...
                        "factor_actualizacion": (
                            data["factor_actualizacion"]
                        ),

                        "importe_actualizacion": (
                            data["importe_actualizacion"]
                        ),
                ...
                # ============================================================
                    ...
                    if not pk_value:
                        errores.append(
                            f"Fila {idx}: Sin PK - SKIP"
                        )
                        continue

                    # ============================================================
                    # 2. ÚLTIMO ABONO MODIFICADO
                    # ============================================================
                    ultimo_abono_modificado = (
                        _calcular_ultimo_abono_modificado(
                            ultimo_abono=row_dict.get("ultimo_abono"),
                            fecha_referencia=fecha_emision_dt
                        )
                    )
                    ...
                    # ============================================================
                    sub_estatus_id = (
                        _calcular_sub_estatus_pensiones(
                            row_dict.get("programa")
                        )
                    )
                ...
                pk_value_log = pk_value if 'pk_value' in locals() else idx
                errores.append(
                    f"Fila {pk_value_log}: {str(e)[:100]}"
                )
...
def get_catalogo_notificadores(
    ...
    try:
        ...
        return []# backend/app/api/analisis.py
from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Query
from sqlalchemy.orm import Session
from sqlalchemy import text
from typing import List, Dict, Any, Optional
