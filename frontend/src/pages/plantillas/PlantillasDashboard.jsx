...
const ICONS = {
  ...
  eye:     { d:"M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z", d2:"M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6z" },
...
export default function PlantillasDashboard() {
  ...
  // Modal editar
  const [editModal,   setEditModal]   = useState(null);  // plantilla completa o null
  const [editForm,    setEditForm]    = useState({ nombre: '', descripcion: '', activa: true });
  const [editSaving,  setEditSaving]  = useState(false);

  // Modal mapeo
  const [mapModal,    setMapModal]    = useState(null);  // plantilla completa
  const [mapData,     setMapData]     = useState({ campos_actuales: [], campos_disponibles: [] });
  const [mapEdits,    setMapEdits]    = useState({});   // placeholder → campo_bd seleccionado
  ...
  const loadPlantillas = useCallback(async () => {
    ...
    try {
      ...
      showMsg('error', 'Error cargando plantillas.');
  ...
  const handleDelete = async (p) => {
    if (!window.confirm(`¿Eliminar plantilla "${p.nombre}"? Esta acción no se puede deshacer.`)) return;
    try {
      await api.delete(`/plantillas/${p.id}`);
      showMsg('success', `Plantilla "${p.nombre}" eliminada.`);
      loadPlantillas();
    } catch (err) {
      showMsg('error', err.response?.data?.detail || 'Error al eliminar.');
    }
  };
  ...
  const openMapeo = async (p) => {
    ...
    try {
      ...
      setMapData(res.data);
      // Pre-llenar edits con valores actuales
      const edits = {};
  ...
  const handleMapSave = async () => {
    ...
    try {
      ...
      showMsg('error', err.response?.data?.detail || 'Error al guardar mapeo.');
    }
  };
    ...
    <div className="pl-page">
      ...
      <div className="pl-filters">
        <div className="pl-filter-group">
          ...
          <select className="pl-select" value={filtroSlug}
        ...
        <div className="pl-filter-group">
          <select className="pl-select" value={filtroActiva}
              ...
              {plantillasFiltradas.map(p => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td>
                      <span className="pl-project-tag">{p.proyecto_nombre}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <span className="pl-campos-count">{p.total_campos} campo{p.total_campos !== 1 ? 's' : ''}</span>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="pl-actions">
                        <button className="pl-icon-btn" title="Ver mapeo de campos"
                          onClick={() => openMapeo(p)}>
                          <Icon {...ICONS.map} size={14} />
                        </button>
                        {isAnalista && (
                          <>
                            <button className="pl-icon-btn" title="Editar"
                              onClick={() => openEdit(p)}>
                        ...
                        <button className="pl-icon-btn" title="Ir a Subir/Crear"
        ...
        <div className="pl-overlay" onClick={() => setMapModal(null)}>
          <div className="pl-modal pl-modal--wide" onClick={e => e.stopPropagation()}>
            ...
            <p className="pl-modal-hint">
              Asocia cada placeholder <code>{'{{campo}}'}</code> de la plantilla con un campo real de <code>tabla_temporal</code>.
            </p>
            <div className="pl-map-grid">
              <div className="pl-map-grid-header">
                <span>Placeholder en plantilla</span>
                <span>Campo en tabla_temporal</span>
              </div>
              {Object.keys(mapEdits).length === 0 && mapData.campos_actuales.length === 0 ? (
                <p className="pl-map-empty">No hay placeholders registrados. Sube una plantilla .docx primero.</p>
              ) : (
                Object.entries(mapEdits).map(([ph, campo]) => (
                  <div key={ph} className="pl-map-row">
                    <span className="pl-map-ph">{ph}</span>
                    <select className="pl-select pl-map-select"
                      value={campo || ''}
            ...
            <div className="pl-modal-footer">
              <button className="pl-btn" onClick={() => setMapModal(null)}>Cerrar</button>
              {isAnalista && (
                <button className="pl-btn pl-btn--primary" onClick={handleMapSave}>
                  Guardar mapeo
                </button>
              )}
            </div>
